...
def get_shorter()
  mini = 99
  mini_i = nil
  $handles.each_with_index.map do |x, i|
    if x.length < mini
      mini = x.length
      mini_i = i
    end
  end
  return $handles[mini_i]
end

def get_5char()
  # sum + (x.length - 1 == 5)
  return $handles.each.inject(0) {|sum, x| sum + (x.length - 1 == 5? 1 : 0)} 
end

def get_1upp()
  # sum + (x.length - 1 == 5)
  return $handles.each.inject(0) {|sum, x| sum + (x[1] == x[1].upcase ? 1 : 0)} 
end

def get_sort_alpha()
  return $handles.sort { |a, b| a[1].downcase <=> b[1].downcase }
end
...
def get_position_epenser()
  return $handles.find_index('@epenser')
end

def get_groupby_size()
  group_by_size = $handles.group_by {|x| x.length}
  size_number = group_by_size.map{|k,v| [k, v.length]}
  size_number.each do |x|
    puts("Number of elements of size #{x[0]}: #{x[1]}")
  end
  return 
end
...
def test()
  ...
  while number != 9
    ...
    puts('1 - number of handles')
    puts('2 - shorter element')
    puts('3 - how many five')
    puts('4 - start with uppercase')
    puts('5 - sort alpha')
    puts('6 - sort by size')
    puts('7 - position of @epenser')
    puts('8 - group by size')
    puts('9 - End')
    ...
    if number == 1
      ...
      res = get_position_epenser()
    elsif number == 7
      res = get_position_epenser()
    elsif number == 8
      res = get_groupby_size()
    elsif number == 9
      return 'Exited' 
    end
    puts res.to_s
